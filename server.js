...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (profileMatch) {
      ...
      const textures = {};
      if (entry.hasSkin) {
        textures.SKIN = { url: `${base}/textures/${uuid}/skin.png` };
        if (entry.slim) textures.SKIN.metadata = { model: "slim" };
      }
      if (entry.hasCape) {
        textures.CAPE = { url: `${base}/textures/${uuid}/cape.png` };
      }
    ...
    // Lista quién está registrado AHORA MISMO en este servidor (sin las
    // imágenes, solo nombre y cuándo se actualizó). Sirve para comprobar en
    // el navegador si el registro de cada persona sigue ahí o si el hosting
    // gratuito reinició el proceso y lo borró (ver nota sobre Render en el
    // README).
    if (url.pathname === "/api/players") {
      const db = loadDb();
      const players = Object.entries(db).map(([uuid, entry]) => ({
        uuid,
        username: entry.username,
        hasSkin: !!entry.hasSkin,
        hasCape: !!entry.hasCape,
        updatedAt: entry.updatedAt,
        secondsAgo: entry.updatedAt ? Math.round((Date.now() - entry.updatedAt) / 1000) : null
      }));
      return send(res, 200, { count: players.length, players });
    }
